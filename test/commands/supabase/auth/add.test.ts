...
describe('auth:add', () => {
  ...
  // tested in plugin-lib's own test suite. Here we only test the Supabase-specific integration.
  it('exports correct integration points', async () => {
    const {default: AuthAdd} = await import('../../../../src/commands/supabase/auth/add.js')
    const {clearClients, testConnection} = await import('../../../../src/supabase/supabase-client.js')
